---
name: commit-message
description: Generate commit messages in the strict semicolon-separated 5-line schema (<jira_key>, type, scope, summary, body) aligned with Change log Process and n8n -> Intercom. Use when the user or agent makes commits via Chat or when asked to suggest a commit message.
---
...
**Limitation:** The built-in "Generate commit message" (icon) button does not use this skill; it applies only when the agent is asked to commit or suggest a message in Chat.
...
5. **Resolve jira_key:** Jira issue key in format `<project>-<id>` (e.g. L3-1234, FIS-456). Order: (1) if the user specified a task -- use it; (2) else try to extract from branch name (e.g. `feature/L3-1234` -> `L3-1234`); (3) else, in interactive Chat, ask the user for the Jira key and **wait for their answer instead of outputting a commit message**; (4) if, after asking, the user does not specify a key, does not know it, or says there is no ticket -- use **NOJIRA**. Validate the Jira key as `<project>-<id>` (for example `ABC-123`) and normalize extracted keys to uppercase so the Bitbucket->Jira link works.

6. **Output the commit message as plain text only (after Step 5 is fully resolved):** If you still need to ask the user for the Jira key, do **not** output a commit message yet. Once `jira_key` is resolved, output with no markdown, no code block, no commentary. Use exactly 5 lines in this order, **with no blank lines between them**: line 1 is `<jira_key>;`, then line 2-5 use field labels (`type:`, `scope:`, `summary:`, `body:`), each followed by a space, value, and semicolon. Ready to paste into `git commit`.
...
**Line 1 (`jira_key`):** Jira issue key in the form **`<project>-<id>`** (e.g. `L3-1234`, `FIS-456`). Use **NOJIRA** when no ticket applies.
...
body: Centralize validation logic for reuse across POS and kiosk to avoid drift.;
```
...
- **Vague summary:** `summary: Fixed the button bug;` -- be specific (which button, what behavior, which context).
- **Missing fields:** omitting the Jira key line or body -- all five lines are required.
...
- Line 1 is **value only** for Jira key: `<jira_key>;` (example: `NOJIRA;`). Do not add a label on this line.
- Lines 2-5 must include field labels: `type: `, `scope: `, `summary: `, `body: `.
- Exactly 5 lines, in order: `<jira_key>;`, `type: ...;`, `scope: ...;`, `summary: ...;`, `body: ...;`. **No blank lines between the five lines** -- each line immediately followed by a single newline (product did not require blank lines; this keeps the format compact and unambiguous for parsing).
...
- **Server:** Bitbucket Push Checkers by regex as a second line of defense.
